      ...
      >
        ...
        <div class="flex items-center justify-between py-1">
          <div class="flex items-center gap-2.5">
            ...
            <span v-if="serie.episodes.filter((e: any) => !e.file_exists).length > 0"
                  class="text-[10px] px-1.5 py-0.5 rounded bg-red-500/10 text-red-400 border border-red-500/20">
              {{ serie.episodes.filter((e: any) => !e.file_exists).length }} introuvable{{ serie.episodes.filter((e: any) => !e.file_exists).length > 1 ? 's' : '' }}
            </span>
            <span v-else-if="serie.needs_rename === 0" class="text-[10px] px-1.5 py-0.5 rounded bg-green-500/10 text-green-400 border border-green-500/20">
              ✓ À jour
            </span>
          </div>
          <div class="flex items-center gap-2">
            <button v-if="serie.needs_rename > 0" @click="renameAll(serie.serie_id)" :disabled="renamingAll" class="text-xs text-accent hover:underline">
              Tout renommer
            </button>
            <button @click="unimportSerie(serie, false)" :disabled="renamingAll" class="text-xs text-muted hover:text-primary transition-colors">
              Désimporter
            </button>
            <button @click="unimportSerie(serie, true)" :disabled="renamingAll" class="text-xs text-red-400 hover:text-red-300 transition-colors">
              Supprimer fichiers
            </button>
          </div>
        </div>
        ...
        <div v-if="!collapsed.has(serie.serie_id)" class="flex flex-col divide-y divide-border/50 mt-2 -mx-4 px-4">
          ...
          >
            ...
            <div class="flex items-center gap-2 shrink-0">
              <span v-if="!ep.file_exists" class="text-[10px] px-1.5 py-0.5 rounded bg-red-500/10 text-red-400 border border-red-500/20">Introuvable</span>
