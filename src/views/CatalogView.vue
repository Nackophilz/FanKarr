    ...
    <header class="sticky top-0 z-50 bg-zinc-950/90 backdrop-blur border-b border-white/5">
      <div class="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
          ...
          <button class="p-2 rounded-lg text-zinc-500 hover:text-zinc-100 hover:bg-white/5 transition" @click="handleLogout">
            <svg width="18" height="18" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
      ...
      <div class="max-w-7xl mx-auto px-6 pb-3 flex flex-wrap items-center gap-3">
        ...
        <div class="flex items-center gap-1">
          <button v-for="f in filtersGroupe1" :key="f.value"
          ...
          <button v-for="f in filtersGroupe2" :key="f.value"
                  class="px-3 py-1.5 rounded-lg text-xs font-medium border transition"
                  :class="activeFilter === f.value
        ...
        <span class="text-xs text-zinc-600">{{ filtered.length }} série{{ filtered.length > 1 ? 's' : '' }}</span>
      </div>
    </header>
      ...
      <div v-else class="grid grid-cols-[repeat(auto-fill,minmax(150px,1fr))] gap-5">
        ...
        >
          <div class="relative aspect-[2/3] rounded-xl overflow-hidden bg-zinc-900 border border-white/5">
            <img v-if="serie.poster_image" :src="serie.poster_image" :alt="serie.title"
            ...
            <div v-if="serie.download_state !== 'none'"
              ...
              {{ serie.download_state === 'complete' ? 'IMPORTÉ' : serie.download_state === 'downloading' ? 'EN COURS' : 'PARTIEL' }}
            </div>
          ...
          <div>
            ...
            <div class="flex items-center gap-1.5 mt-0.5">
              ...
              <span v-if="serie.status" class="text-[10px] px-1.5 py-0.5 rounded"
                    :class="serie.status.toLowerCase() === 'continuing'
                    ? 'bg-green-500/10 text-green-400'
                    : 'bg-white/5 text-zinc-600'">{{ serie.status }}</span>
            </div>
          </div>
        </router-link>
      </div>
...
const filtered = computed(() => {
  ...
  switch (activeSort.value) {
    ...
    case 'recent':
      ...
      break
    case 'imported':
      // Importés en premier, puis partiel, puis reste
...
async function fetchOrganizeNotifs() {
  try {
    ...
    // Rafraîchir le catalogue si de nouveaux imports ont eu lieu
